...
# Force CPU and disable mixed precision for consistency
os.environ["CUDA_VISIBLE_DEVICES"] = "-1"  # Disable CUDA
os.environ["TORCH_DTYPE"] = "float32"  # Force float32

# Setup logging
logging.basicConfig(level=logging.INFO, format='[%(asctime)s %(name)s %(levelname)s] %(message)s')
logger = logging.getLogger("roma_matcher")

# Constants - Force CPU for memory efficiency in Docker
DEVICE = torch.device("cpu")  # Always use CPU to avoid memory issues
...
CHECKPOINTS_DIR = PROJECT_ROOT / "ROMA_checkpoints/roma"
...
class RomaMatcher(nn.Module):
    ...
    def __init__(self, memory_efficient=True):
        ...
        if memory_efficient:
            self.conf = {
                ...
                "max_keypoints": 200,  # Keep reduced for Docker
                "coarse_res": (294, 294),  # 294 = 21 * 14, multiple of 14
                "upsample_res": (392, 518),  # 392 = 28 * 14, 518 = 37 * 14  
                "max_image_size": 420,  # 420 = 30 * 14, multiple of 14
            }
        else:
            self.conf = {
                "model_name": "gim_roma_100h.ckpt",
    ...
    def match_images(self, image0, image1):
        ...
        with torch.no_grad():
            ...
            # Ensure we're using CPU and float32
