...
class AerialImageryMatcher:
    ...
    def _load_roma_model(self):
        ...
        try:
            ...
            self.roma_model = RomaMatcher(memory_efficient=True)
            self.roma_available = True
            logger.info("ROMA model loaded successfully (memory-efficient mode)")
        ...
        try:
            ...
            if output_dir:
                ...
                if os.path.exists(dest_pgw_path):
                    # Implement georeferencing (simplified for now)
                    logger.info("Georeferencing functionality available but simplified in this refactor")
